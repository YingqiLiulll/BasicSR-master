...
import timm
from basicsr.pg_modules.blocks import FeatureFusionBlock
...
def calc_channels(pretrained, inp_res=224):
    ...
    channels.append(tmp.shape[1]) #so we can get the channel number after layer0
    tmp = pretrained.layer1(tmp)
    channels.append(tmp.shape[1]) #layer1
    tmp = pretrained.layer2(tmp)
    channels.append(tmp.shape[1]) #layer2
    tmp = pretrained.layer3(tmp)
    channels.append(tmp.shape[1]) #layer3
    # return channels[layer0,layer1,layer2,layer3]
