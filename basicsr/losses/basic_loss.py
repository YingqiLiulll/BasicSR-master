...
class class_loss_3class(nn.Module):
    ...
    def forward(self, type_res, **kwargs):
        n = len(type_res)
        m = len(type_res[0]) - 1
        type_all = type_res
        loss = 0
        for i in range(n):
            ...
            loss += (m - sum_re)
        class_loss = loss/n
        return self.loss_weight * class_loss
...
class average_loss_3class(nn.Module):
    ...
    def forward(self, type_res):
        ...
        average_loss = (abs(sum1-n/m) + abs(sum2-n/m) + abs(sum3-n/m)) / ((n/m)*4)

        return  self.loss_weight * average_loss
